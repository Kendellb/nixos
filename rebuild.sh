...
# Prompt the user to choose the host
echo "Please select the host: (vm, laptop, desktop)"
read -r host

# Validate the input (ensure it's one of the allowed values)
if [[ "$host" != "vm" && "$host" != "laptop" && "$host" != "desktop" ]]; then
    echo "Invalid host name. Please choose from 'vm', 'laptop', or 'desktop'. Exiting."
    exit 1
fi

# Edit your config
$EDITOR ~/nixos/hosts/$host
...
alejandra . -e ./hosts/$host/hardware-configuration.nix &>/dev/null \
  || ( alejandra . -e ./hosts/$host/hardware-configuration.nix; echo "formatting failed!" && exit 1)
